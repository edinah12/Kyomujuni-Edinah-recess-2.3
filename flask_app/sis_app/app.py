...
if __package__:
    from .database import delete_student, get_student_by_registration, init_db, add_student, get_all_students, get_student_by_name, update_student
...
@app.route('/students/edit/<registration_number>', methods=['GET', 'POST'])
def edit_student(registration_number):
    ...
    if request.method == 'POST':
        ...
        success, message, _ = update_student(registration_number, student_data)
...
@app.route('/students/delete/<registration_number>', methods=['POST'])
def delete_student_route(registration_number):
    success, message = delete_student(registration_number)
    flash(message, 'success' if success else 'error')
    return redirect(url_for('students_list'))
