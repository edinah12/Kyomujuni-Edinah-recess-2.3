...
def get_student_by_registration(registration_number):
    return Student.query.filter_by(registration_number=registration_number, is_active=True).first()
...
def update_student(registration_number, student_data):
    try:
        ...
        if existing_email:
            return False, "Email already registered by another student!", None
